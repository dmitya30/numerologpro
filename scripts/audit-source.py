...
def fetch(url: str) -> tuple[int, str, bytes, str]:
    ...
    try:
        ...
        if completed.returncode != 0:
            diagnostic = normalize_space(completed.stderr)
            raise RuntimeError(
                f"curl завершился с кодом {completed.returncode}: {diagnostic}"
            )
        if len(metadata) != 3:
            raise RuntimeError(
                "curl вернул неожиданный формат metadata: "
                + repr(completed.stdout)
            )

        status_text, content_type, final_url = metadata
        if not status_text.isdigit():
            raise RuntimeError(
                "curl вернул некорректный HTTP status: "
                + repr(status_text)
            )

        body = Path(temporary_path).read_bytes()
        return int(status_text), content_type, body, final_url
    finally:
        if temporary_path:
